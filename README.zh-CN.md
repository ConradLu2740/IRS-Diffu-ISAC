...
| 1 步蒸馏 FM 学生 | C1 HRRP 条件 FM 渐进蒸馏——matched budget 最终裁定：CD 比 teacher NFE=1 引导均值**差 118%**（0.5056 vs 0.2318；第 26 轮的 −23.4% 是小预算欠拟合假象）；蒸馏买到的是**多样性**：pairwise 0.0053→0.1127（比率 0.02→0.48），随训练预算与 teacher 积分阶数 K 单调上升（**K=4 是实用工作点**：多样性较 K=2 +31%、CD 仅 +3%；K>4 边际递减）；1 步**采样器**（数据增广/不确定性估计），CD 驱动的闭环形状先验仍用均值（定理：NFE=1 CFM 输出=条件均值）；`make train-fm-distill-full` / `make verify-fm-distill-diversity` |
| OTFS/AFDM vs 真实多普勒 | ICI 恒等式 **28.35%** @ ±611 kHz（MC 10⁶）；OTFS BER **0** vs OFDM 7.7e-2（同 SNR）；ISAR 冻结几何阈值 32.3 Hz vs 实际 ≥2.53 kHz（**78× 违背**） |
| 条件-形状数据审计（N1–N3） | 训练分布上近同条件仍对应差异 0.66 raw / 0.027 aligned 的点云——teacher 坍塌散布的 **125× / 5×**：过坍塌是建模失败而非贝叶斯正确（学生宽 9× 仍差真实条件宽度 13.6×）；条件的成对形状分辨力为**零**（ρ=−0.015：距离剖面多对一）；`make verify-cond-diversity` |
