...
**一句话头条**：在 ELBO 一致性训练下，1 步 Flow Matching 采样器追平 100 步扩散（CD −9%，在运行间方差内），10 步采样器稳定超出 26–36%；整个闭环可证地达到**认证全局最优的 ≈69%**（Bootstrap 95% CI [0.638, 0.745]；FM 形状先验升级后 ≈74%，v1.14）。
