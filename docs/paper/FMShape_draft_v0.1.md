...
\tag{4}
$$
whose induced conditional velocity is the constant vector $\mathbf{x}_1 - \mathbf{x}_0$. The flow network $v_{\boldsymbol{\theta}}$ — a 1-D Diffusion Transformer with cross-attention (2 blocks, 256 hidden units, 8 heads) — is trained with the regression objective (5) of [17]
$$
...
**E. One-step sampling.** Inference integrates the probability-flow ODE $\dot{\mathbf{x}} = v_{\mathrm{cfg}}(\mathbf{x}, t)$ from $t = 0$ to $t = 1$ with a numerical solver. The real-time operating point is a single Euler step,
$$
...
\tag{8}
$$
Section VI quantifies the effect of (8): the augmentation adds channel capacity only under spread equalization; without it, the augmentation is actively harmful.

**G. Training protocol.** All generative models are trained with the fresh-data protocol: each epoch draws new samples from the physics-based simulator, and no dataset is materialized or frozen. This matters for conditional generation in particular — freezing the data lets the network memorize the finite sample and collapse the condition dependence, an artifact documented in our earlier work and avoided here by construction. Models are trained with identical architectures, capacities, and data budgets whenever compared, and all results are reported on a common test batch with fixed seeds so that paired comparisons are not confounded by evaluation noise.
...
| [9] | Y. Lipman et al., "Flow matching for generative modeling," in *Proc. ICLR*, 2023 |
...
| [12] | M. M. Rahman, S. Z. Gurbuz, and M. G. Amin, "Physics-aware generative adversarial networks for radar-based human activity recognition," *IEEE TAES*, 59(3):2994–3008, 2023 |
| [13] | Y. Bao et al., "Improved few-shot SAR image generation by enhancing diversity," *IEEE JSTARS*, 17:3394–3408, 2024 |
...
| [17] | A. Tong et al., "Conditional flow matching: Simulation-free dynamic optimal transport," arXiv:2302.00482, 2023 |

---

## Venue 适配说明（TAES 终判）

**主投：IEEE Transactions on Aerospace and Electronic Systems (TAES)**。理由：(i) 主题最契合——雷达感知观测量（HRRP/ISAR）上的生成式形状重建，审稿人是雷达+信号处理社区；(ii) 团队在 ISAC 方向的积累（卫星通感一体化、Intelligent ISAC 综述）提供领域可信度；(iii) TAES接受有强可复现性的仿真平台论文（本文附开源代码+固定种子+预注册裁决）。

**格式要点**：双栏；正文建议 ≤10 页（现草稿约 10–12 页等价，投稿前需压图或移附录）；摘要 ≤250 词 ✓；Index Terms 3–5 个 ✓；缩写首现全称 ✓；公式按 (1)–(13) 顺序编号 ✓；表 I–III、图 1–3 均已在文中引用 ✓。
**EDICS 建议**：Sensing; Radar signal processing; Machine learning。
**已知风险与缓解**：无实测数据是 TAES 常见拒稿点——缓解：强调 SGP4 真实星历 + 物理信道 + SDR 接口 + 代码开源（第六节 G 小节已声明），并在 cover letter 主动说明仿真定位。
**备选路径**：若 TAES 以“缺实测数据”拒稿 → 改投 IEEE IoT-J（团队常发，ISAC 系统叙事，题目加 “for ISAC Systems” 后缀即可，闭环一节提前）；若想强化理论 → IEEE TSP（第五节扩展为更完整的收敛分析）。
