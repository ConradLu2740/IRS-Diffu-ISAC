...
def train_PointVAE(vae, train_loader, test_loader, device="cuda", epochs=100, lr=1e-3, kl_weight=1e-6, save_dir="./model"):
    ...
    for ep in range(1, epochs + 1):
        ...
        print(f"[Point-VAE-RandomIRS] epoch {ep}/{epochs} train_total={train_total:.6f} test_total={test_total:.6f} train_cd={train_cd:.6f} test_cd={test_cd:.6f} train_kl={train_kl:.6f} test_kl={test_kl:.6f}")
    ...
    save_loss_curve(history["train_total"], history["test_total"], title="PointVAE Random-IRS Loss Curve", save_path=os.path.join(save_dir, "vae_loss_curve.png"))
...
# =========================================================
# 单帧特征构造
# x: [16,1] complex -> 32 dim
# y: [16,1] complex -> 32 dim
# phase: 32 -> sin/cos -> 64 dim
# 每帧总计 128 dim
# =========================================================
...
def rollout_physical_sequence(
    ...
    for _ in range(Tau):
        ...
        # 每个时间步、每个样本都重新随机生成一组相位
        phase_t = 2.0 * torch.pi * torch.rand(B, setup.IRS_total, device=device)
...
def train_ldm(
    ...
    beta_start=1e-4,
    beta_end=2e-2,
    save_dir="./outputs_ldm_random_irs/check",
):
    os.makedirs(save_dir, exist_ok=True)
    ...
    for ep in range(start_ep, epochs + 1):
        ...
        print(f"[LDM-RandomIRS] epoch {ep}/{epochs} train_loss={train_loss:.6f} val_loss={val_loss:.6f}")
    ...
    save_loss_curve(
        ...
        title="LDM Random-IRS Loss Curve"
    )
...
def main(
    ...
    num_points=2048,
    epochs=100,
    save_dir="./outputs_ldm_random_irs",
):
    cfg = {
        "device": device,
    ...
    print("[Step] start building ROILDMDataset")
    ...
    print("[Step] finish building ROILDMDataset")
    ...
    train_dataset = CachedLatentDataset(
        ...
        vae,
        device=device,
        print_prefix="TrainLatentCache-RandomIRS"
    )
    ...
    val_dataset = CachedLatentDataset(
        ...
        vae,
        device=device,
        print_prefix="ValLatentCache-RandomIRS"
    )
