...
| `verify_fm_distill_diversity.py` | D1：蒸馏学生 vs teacher 样本多样性（同 cond × 16 噪声 + CFG 尺度扫描，模式坍塌机制） | `python verify_fm_distill_diversity.py --cfg_list 0 1 2` | `sat_model_distill/`（多样性 + CFG 扫描 JSON） |
| `verify_cond_shape_diversity.py` | N1/N2/N3：条件-形状多样性审计（数据本身有无同条件不同形状；HRRP 成对形状分辨力） | `python verify_cond_shape_diversity.py --n_samples 96` | `sat_model_c1/`（审计 JSON） |
