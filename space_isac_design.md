...
`compute_isar_sequence`：目标自转 ω（如卫星自旋/无人机盘旋），M 帧宽带距离像 → 距离-时间序列。

| 配置 | 分类 acc | 姿态 MAE（纯任务） |
|------|---------|------------------|
| 窄带（4天线+8帧） | 0.383 | ~90°（不可行） |
| 宽带 1D 距离像 | 0.867 | 29.1° |
| **ISAR 多帧序列** | **0.933** | **21-25°** |

物理规律（参数扫描）：
- ISAR 合成转角越大姿态越可估：1.2°→28.8°、6.2°→22.0°、45°→21.3°
- 收益递减受限于目标对称性（uav/cubesat 旋转对称 → 姿态模糊上界）
- 多任务（分类+姿态）下分类主导；纯姿态任务可达 21-25°（任务分离/独立编码器是改进方向）
