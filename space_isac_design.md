...
|------|--------|------------|-----------|-----------|
| **2D-CFAR**（经典） | **1.000** | **6.98 m** | —（无角度分辨力） | — |
| **MUSIC**（ULA 8） | — | — | — | — |
| **ML（绝对距离特征）** | — | **3.09 m** | 15.68 m | 0.650 |
| **ML（旧特征=类别先验）** | — | — | 2D 20.43 m | 0.800 |

MUSIC 目标方向测向 MAE **0.017°**（ULA 高精度），但对 ROI 内目标定位无信息。

### 重要发现（物理 + 工程）

1. **特征构造缺陷（工程 bug）**：`data_sat.compute_range_profile` 的 d_proj 相对**体素质心**
   （`rel = p - p_center`），目标在 ROI 中的**绝对位置信息在特征层被丢弃**——
   实测不同位置的单体素 RP 质心 bin 恒定。旧 ML 定位≈类别先验（2D RMSE 20.4 m）；
   改用绝对距离特征后沿视线 RMSE 降至 3.1 m。
