...
> ⚠️ **历史值警示（2026-08-12）**：下表为早期（修复前）结果。当前可复现的逐帧跟踪功率提升为 **+89%**（K=1）、K=2: +60.0%、K=4: +36.6%、K=8: **−41.5%**（详见 TECH_REPORT v1.3 / README Key Results）。旧数字仅早期 commit 可复现。
...
### 关键结果
- 10 目标 5 类：检测召回 **0.812**（⚠️ 历史值，修复前结果；当前可复现为 **0.60**，见 TECH_REPORT v1.3 §4.2）、类别准确率（原始）0.518 → 轨迹聚合后显著提升
