...
### 6.28 C3: ISAR Conditioning Surfaces a Methodological Trap (v1.33)

Registered proposition C3 augments the condition with the ISAR slow-time Doppler profile (HRRP + 32-dim
Doppler, 544 dims). Two experiments:

**Experiment 1 (trained model, Delta test)**: Delta ~ 0 — the condition encoder collapsed again.
Root cause: this run materialized the dataset once (to afford the expensive per-sample ISAR
computation), freezing the 1024 samples; the model memorized them, the condition became unnecessary,
and the encoder collapsed. Methodological lesson: in conditional generative training, data freezing ->
memorization -> condition collapse. C1 succeeded precisely because its data were regenerated per epoch;
materialization is only safe for unconditional/discriminative tasks.

**Experiment 2 (direct information test, no training confound)**: the ISAR slow-time Doppler profile
predicts the 5 target classes at 40.5% +/- 4.7% (chance 20%) — genuine new information beyond HRRP,
but weaker (HRRP gives 0.80+).
...
### 6.36 The CFG Sweep: the Collapse Is Intrinsic, Guidance Is Pure Profit for the Teacher, the Student Is a Different Operating Point (v1.41)

The v1.40 audit left a mechanism question: is the teacher's near-collapse a CFG artifact? `verify_fm_distill_diversity.py --cfg_list 0 1 2` sweeps the guidance scale on the same batch with a paired x0 (seed 999); the re-run reproduced the Section 6.35 headline numbers exactly (0.2318 / 0.4950 / 0.3839 — deterministic protocol check):

| w | Teacher CD | Teacher ratio | Student CD | Student ratio |
|---|---|---|---|---|
...
| Proposition | Prediction | Measured | Verdict |
|---|---|---|---|
| M1 CFG drives collapse: ratio(w=0) ≥ 3× ratio(w=2) | removing guidance restores diversity | 0.02 vs 0.02 | **FAIL** |
| M1b guidance buys quality: CD(w=2) ≤ CD(w=0) | guidance improves quality | 0.2664 ≤ 0.3060 | **PASS** |
...
### 6.39 The Full-Scale Retrain: Both P1 and P2 Fail at Matched Budget, the Bias–Variance Frontier Along the Budget Axis (v1.44)

The v1.39 budget caveat is closed with the `--eval_seed` protocol: the full-scale student (1024/100) and the
round-26 student (512/60) are compared on one common test batch (teacher numbers reproduce bit-exactly):

| Model | Budget | CD-to-GT | Pairwise CD (16×x0) | Diversity ratio |
|---|---|---|---|---|
| Teacher NFE=1 (= conditional mean, T2) | — | **0.2318** | 0.0053 | 0.02 |
| Teacher NFE=2 (2-step sampler) | — | 0.4950 | — | — |
| Student | 512/60 | 0.3839 (+65.6%) | 0.0488 | 0.19 |
| Student | 1024/100 | 0.5056 (**+118.1%**) | **0.1127** | **0.48** |
| GT true conditional width (§6.37 closest decile) | — | 0.6634 | — | — |

Verdicts: **P1 FAIL, P2 FAIL, P3 PASS** (loss 1.794 → 0.0958, a far better fit than the small budget's 0.2535).
The v1.39 −23.4% advantage not only vanishes at matched budget — it inverts sign: the small-budget run's
apparent win was underfitting that averaged the student back toward the mean. P2 also narrowly fails (0.5056
vs 0.4950): the 1-step approximation of the 2-step map adds its own error on top.

This is the Section 6.38 bias–variance frontier measured along the training-budget axis: budget ↑ → fidelity to
the 2-step sampler ↑ → diversity ↑ monotonically (0.0488 → 0.1127; 21× the teacher) and CD ↑ monotonically
(0.3839 → 0.5056); the full-budget student's spread reaches 17% of the empirical conditional width (7% at the
small budget). Final positioning of the generative line (all four segments closed, no unverified strong
claims): the closed-loop shape prior uses the teacher NFE=1 guided mean (CD 0.2318, CD-Bayes-optimal); the
...
### 6.41 The C3 Correction: the Dilution Is Unequal Block Spread, and Distance-Scale Equalization Recovers 97% of the Signal (v1.46)

The v1.45 registered correction is tested — and corrects itself. The first proposed fix (per-block unit
normalization) is a no-op: `data_sat.py` already unit-norms every block, and the measured numbers are
bit-identical before and after (ρ = 0.002 both ways). The actual dilution mechanism is unequal per-block
pairwise spread: HRRP mean pairwise distance 0.9448 vs Doppler 0.1884 (5.0×) — the 512-dimensional,
near-orthogonal HRRP dominates the L2 neighbor structure. The operative fix divides each block by its own mean
pairwise distance (block standardization):

| Condition | ρ | Closest-decile aligned CD |
|---|---|---|
| HRRP only | −0.015 | 0.0265 |
| C3 concatenated (raw) | 0.002 | 0.0264 |
| C3 concatenated (unit-norm — the v1.45 no-op) | 0.002 | 0.0264 |
| **C3 concatenated (spread-equalized, 5.0×)** | **0.205** | **0.0221** |
| Doppler only | 0.212 | 0.0222 |

Pre-registered checks: S1 (ρ ≥ 0.5× dop) PASS, S2 (> 3× raw concatenation) PASS. Spread equalization recovers
97% of the Doppler profile's standalone discriminative signal, and the conditional-width narrowing matches the
dop-only operating point. The C3 training protocol is thereby fixed before any compute is spent: spread-
equalized block concatenation, not raw or unit-norm concatenation — without it, C3-as-concatenation is
HRRP-only and reproduces the C2 dilution.

### 6.42 R1 Complete: the K-Axis of the Bias–Variance Frontier — Predictions All Confirmed, Diminishing Returns Beyond K=4 (v1.47)

Registered experiment R1: at fixed budget (512/60), distilling from K-step teacher maps for K ∈ {2, 4, 10} on
the common eval batch (teacher numbers reproduce bit-exactly):

| K (teacher integration steps) | Student CD | vs teacher1 | Pairwise CD | Diversity ratio | Regression loss |
|---|---|---|---|---|---|
| 2 | 0.3839 | +65.6% | 0.0488 | 0.19 | 0.2535 |
| 4 | 0.3956 | +70.6% | 0.0640 | 0.27 | 0.3785 |
| 10 | 0.4368 | +88.4% | 0.0687 | 0.29 | 0.3822 |

The T3/T4 predictions are all confirmed: diversity rises monotonically with K (0.0488 → 0.0640 → 0.0687) and
CD worsens monotonically (0.3839 → 0.3956 → 0.4368) — the higher the order of the regressed map, the more
dispersion the student inherits and the larger the bias–variance price. Two operational findings: (i)
diminishing returns beyond K=4 — diversity gains only 7% from K=4→10 while CD pays another 10.4%, so K=4 is
the practical sampler operating point; (ii) the regression loss grows with K (0.2535 → 0.3785 → 0.3822)
because higher-order maps are more x0-dependent and harder for a 1-step student to fit, which also explains
why the 1024/100 budget was only necessary at K=2 (§6.39). P1 fails at every K (the CD cost vs the mean is
the structural T4 price); P2 passes at K=4/10 and fails only for the well-fit K=2 full-budget run, so the
student's CD position is set by its fit to the K-step map. The frontier is now complete on both axes
(budget, K).

### 6.43 The C3 Full-Scale Run: the Equalization Fix Transfers to the Network Side, but Δ(0) Stays Below C1 (Budget-Confounded; Control Running) (v1.48)

The C3 full-scale training run executes the Section 6.41 protocol with fresh data at 512/60 (budget below
C1's 1024/100, recorded as a deviation), training two models: raw HRRP+dop concatenation and
spread-equalized concatenation, sharing the VAE:

| Model | FM NFE=1 CD | Δ(0) | max\|Δ\| | Identity resid. | Cond. encoder |
|---|---|---|---|---|---|
| C3-raw | 0.4011 | **0.131** | 0.195 | 1.4e-07 | not collapsed (output std 4.5× input) |
| C3-eq | 0.4065 | **0.154** | 0.215 | 1.7e-07 | not collapsed |
| C1 (HRRP only, 1024/100) | 0.2269 | 0.302 | — | 1.6e-07 | not collapsed |

| Proposition | Prediction | Measured | Verdict |
|---|---|---|---|
| C3a equalization transfers to the network side | Δ(0)(eq) > Δ(0)(raw) | 0.154 vs 0.131 (**+17.6%**) | **PASS** |
| C3b ISAR channel beats the HRRP channel | Δ(0)(eq) ≥ 1.1× C1 | 0.154 vs 0.302 | **FAIL** (budget-confounded, below) |
| C3c no quality regression | CD(eq) ≤ 1.1× CD(raw) | 0.4065 vs 0.4011 (+1.3%) | **PASS** |

Three conclusions. (i) The equalization fix transfers: +17.6% Δ(0) on the network side matches the data-level
pre-screen direction, so the C2-style dilution is genuinely repairable on both the metric side and the
learning side. (ii) The ISAR condition opens a real but modest information channel: Δ(0) = 0.131/0.154 clears
the 0.05 threshold, the encoder is not collapsed, and the null A/B difference (0.25) excludes undertraining
explanations; Δ(t) is non-monotone — structurally different from C1's monotone decrease. (iii) C3b fails as
registered but is budget-confounded: C1's 0.302 came at 1024/100 versus C3's 512/60, so the honest reading is
"half-budget ISAR does not beat full-budget HRRP", not "ISAR is useless". A matched-budget HRRP-only control
...
11. **The generative model at its real-time operating point is a Bayes estimator, not a sampler** (Sections 6.38–6.39): for OT-CFM the NFE=1 Euler output is provably the conditional mean μ(c), so the HRRP-conditioned FM's 1-step map is near-mode-collapsed by construction (measured diversity ratio 0.02) — this is optimal behavior for the Chamfer metric, not a defect. Sample diversity requires multi-step integration or progressive distillation, and any dispersed sampler pays a structural CD cost (measured +66% to +118% vs the mean; the bias–variance frontier is mapped along both the training-budget and teacher-integration axes, K=4 being the practical distillation operating point). The earlier "1-step student beats its teacher by 23.4%" headline is superseded: it does not replicate at matched budget.
12. **The HRRP condition cannot narrow the posterior** (Sections 6.37, 6.40–6.41): the range profile is a many-to-one map with ~zero pairwise shape-discriminative power (Spearman ρ = −0.015), so conditioning shifts the conditional mean (CD −14%) while the conditional width — ROI position plus cross-range shape, empirically 125×/5× the collapsed spread — remains unrecoverable from HRRP. The ISAR slow-time Doppler profile carries real but modest additional shape information (ρ = 0.212), and only under spread-equalized block concatenation (raw concatenation dilutes it to ρ = 0.002, the C2 failure mode at the data level). Conditional generative sensing in this scenario is therefore bounded by the observation, not by the model.
13. **Training-budget sensitivity is real and was itself a source of error**: the round-26 distillation claim inverted sign between the 512/60 and 1024/100 budgets, and the ISAR condition path silently materialized data until fixed (the pre-fix C3 run is void, preserved as `sat_model_c3_mat/`). Single-budget, single-batch comparisons at n_eval=8 have repeatedly proven unreliable in this repo; the `--eval_seed` common-batch protocol is the minimum standard for paired claims.
