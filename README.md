...
| **1-step distilled FM student** | Progressive distillation of the C1 HRRP-conditional FM — final verdict at matched budget: CD **+118% worse** than the teacher's NFE=1 guided mean (0.5056 vs 0.2318; the round-26 −23.4% was small-budget underfitting); what distillation buys is **diversity**: 0.0053 → 0.1127 pairwise (ratio 0.02 → 0.48), monotone in training budget and in teacher-integration order K (K=4 is the practical operating point — diversity +31% over K=2 for +3% CD; beyond K=4 returns diminish); a 1-step *sampler* for augmentation/uncertainty, while the CD-driven closed-loop shape prior stays on the mean (theorem: NFE=1 CFM output = conditional mean); `make train-fm-distill-full` / `make verify-fm-distill-diversity` |
...
| **Condition–shape data audit (N1–N3)** | On the training distribution, near-identical HRRP conditions still map to clouds differing 0.66 raw / 0.027 aligned — **125× / 5× the teacher's collapsed spread**: the over-collapse is a modeling failure, not Bayes-correct (the student's 9× wider spread is still 13.6× short of the true conditional width); the condition's pairwise shape-discriminative power is **zero** (ρ = −0.015 — the range profile is many-to-one); `make verify-cond-diversity` |
