...
| **1-step distilled FM student** | Progressive distillation of the C1 HRRP-conditional FM: the CD advantage is **batch-specific** (−23.4% on one batch, +65.6% worse on an independently seeded one); the robust finding is **diversity** — the NFE=1 teacher is *intrinsically* near-mode-collapsed (ratio 0.02 at every CFG scale w=0/1/2: a conditional-mean estimator; guidance w=2 is pure quality profit, CD 0.306→0.266) while the student restores **9×** sample diversity (its ratio rises with w — inverted vs ordinary CFG) at a CD cost: a different quality–diversity operating point, not a free upgrade; `make train-fm-distill` / `make verify-fm-distill-diversity` |
